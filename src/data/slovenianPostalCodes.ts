/**
 * Slovenian Postal Codes Database
 * Maps postal codes to city/town names for autofill functionality
 */
...
// Most common Slovenian postal codes
export const SLOVENIAN_POSTAL_CODES: Record<string, string> = {
  // Ljubljana region
  '1000': 'Ljubljana',
  ...
  // Prekmurje (relevant for Kmetija Maroša)
  '9000': 'Murska Sobota',
  '9201': 'Puconci',
  '9202': 'Mačkovci',
  '9203': 'Petrovci',
  ...
  '9227': 'Kobilje',
  '9231': 'Beltinci',  // Kmetija Maroša location!
  '9232': 'Črenšovci',
  '9233': 'Odranci',
  '9240': 'Ljutomer',
  '9241': 'Veržej',
...
export function getCityFromPostalCode(postalCode: string): string | null {
  ...
  return SLOVENIAN_POSTAL_CODES[cleanCode] || null;
}

/**
 * Validate if postal code is valid Slovenian format (4 digits)
 */
export function isValidSlovenianPostalCode(postalCode: string): boolean {
  const cleanCode = postalCode.replace(/\s/g, '').trim();
  return /^\d{4}$/.test(cleanCode) && cleanCode in SLOVENIAN_POSTAL_CODES;
}

/**
 * Search postal codes by city name
 */
export function searchPostalCodesByCity(cityName: string): PostalCodeData[] {
  const results: PostalCodeData[] = [];
  const searchTerm = cityName.toLowerCase().trim();
  
  for (const [code, city] of Object.entries(SLOVENIAN_POSTAL_CODES)) {
    if (city.toLowerCase().includes(searchTerm)) {
      results.push({ postalCode: code, city });
    }
  }
 ...
 */
export function getAllPostalCodes(): PostalCodeData[] {
  return Object.entries(SLOVENIAN_POSTAL_CODES).map(([postalCode, city]) => ({
    postalCode,
    city
  }));
}
